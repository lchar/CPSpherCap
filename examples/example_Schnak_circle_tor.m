...
%log: Toroidal coordinates growth (increasing xi)
...
[unew, flagu] = gmres(Au,uc - 2*dxi*dt/R*cpy.*uc + dt.*(a - uc + uc.*uc.*vc), 10, tol, maxit);%, Lu, Uu, u);
[vnew, flagv] = gmres(Av,vc - 2*dxi*dt/R*cpy.*vc + dt.*(b - uc.*uc.*vc), 10, tol, maxit);
uold = u;
u = unew;
vold = v;
v = vnew;
...
for kt = 2:numtimesteps
  ...
  % New Operators
  Enew = interp2_matrix(x1d, y1d, cpx, cpy, p, bandnew);
  E = interp2_matrix(x1d, y1d, real(xf(eta, xi - dxi*dt)), real(yf(eta, xi - dxi*dt)), p, band);
  Eold = interp2_matrix(x1d, y1d, real(xf(eta, xi - 2*dxi*dt)), real(yf(eta, xi - 2*dxi*dt)), p, bandold);
  I = speye(size(Enew));
  L = laplacian_2d_matrix(x1d,y1d, order, bandnew);
  ...
  % TO DO: use correct y values instead of cpy
  [unew, flagu] = gmres(Au,(2*uc - 1/2*uoldc) - 2*2*dxi*dt/R*cpy.*uc + 2*dxi*dt/R*cpy.*uoldc + 2*dt.*(a - uc + uc.*uc.*vc) - dt.*(a - uoldc + uoldc.*uoldc.*voldc), 10, tol, maxit);%, Lu, Uu, u);
  [vnew, flagv] = gmres(Av,(2*vc - 1/2*voldc) - 2*2*dxi*dt/R*cpy.*vc + 2*dxi*dt/R*cpy.*voldc + 2*dt.*(b - uc.*uc.*vc) - dt.*(b - uoldc.*uoldc.*voldc), 10, tol, maxit);
