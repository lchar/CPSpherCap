...
[unew, flagu] = gmres(Au,uc - k*dt.*uc + dt.*(a - uc + uc.*uc.*vc), 10, tol, maxit);%, Lu, Uu, u);
[vnew, flagv] = gmres(Av,vc - k*dt.*vc + dt.*(b - uc.*uc.*vc), 10, tol, maxit);
uold = u;
u = unew;
vold = v;
v = vnew;
...
for kt = 2:numtimesteps
  ...
  [unew, flagu] = gmres(Au,(2*uc - 1/2*uoldc) - 2*k*dt.*uc + k*dt.*uoldc + 2*dt.*(a - uc + uc.*uc.*vc) - dt.*(a - uoldc + uoldc.*uoldc.*voldc), 10, tol, maxit);%, Lu, Uu, u);
  [vnew, flagv] = gmres(Av,(2*vc - 1/2*voldc) - 2*k*dt.*vc + k*dt.*voldc + 2*dt.*(b - uc.*uc.*vc) - dt.*(b - uoldc.*uoldc.*voldc), 10, tol, maxit);
