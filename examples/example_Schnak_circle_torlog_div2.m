...
%torlog_div2: Toroidal coordinates growth following logistic growth (increasing xi)
%               Including numerical divergence computation.
%               Including velocity field backtracking.
...
Vx = real(dxdxif(eta, xif(dt)))*real(dxif(dt));
Vy = real(dydxif(eta, xif(dt)))*real(dxif(dt));
...
E = interp2_matrix(x1d, y1d, cpx - dt*Vx, cpy - dt*Vy, p, band);
I = speye(size(Enew));
...
for kt = 2:numtimesteps
  ...
  Vxnew = real(dxdxif(etanew, xif(t)))*real(dxif(t));
  Vynew = real(dydxif(etanew, xif(t)))*real(dxif(t));
  ...
  % New Operators
  Enew = interp2_matrix(x1d, y1d, cpx, cpy, p, bandnew);
  E = interp2_matrix(x1d, y1d, cpx - dt*Vxnew, cpy - dt*Vynew, p, band);
  Eold = interp2_matrix(x1d, y1d, cpx - 2*dt*Vxnew, cpy - 2*dt*Vynew, p, bandold);
  I = speye(size(Enew));
  L = laplacian_2d_matrix(x1d,y1d, order, bandnew);
