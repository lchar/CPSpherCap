...
dx = 0.020;  % grid spacing ###May crash if too small###
R=1; %Radius of the circle
gamma = 0.499;      % shape parameter
...
%% Banding: do calculation in a narrow band around the surface
  ...
  band = find(abs(dist) <= bw*dx | abs(distf) <= bw*dx);
...
% iCPM matrix
%M = lapsharp(L,E, 6/dx^2);
%M = lapsharp(L,E);
M = E*L - 6/dx^2*(speye(size(E))- E);
