...
E = interp2_matrix(x1d, y1d, g(0)/g(dt).*cpx, g(0)/g(dt).*cpy, p, band);
I = speye(size(Enew));
...
[unew, flagu] = gmres(Au,uc - k*dt*(1 - g(dt)/xi).*uc + dt.*(a - uc + uc.*uc.*vc), 10, tol, maxit);%, Lu, Uu, u);
[vnew, flagv] = gmres(Av,vc - k*dt*(1 - g(dt)/xi).*vc + dt.*(b - uc.*uc.*vc), 10, tol, maxit);
uold = u;
u = unew;
vold = v;
v = vnew;
...
for kt = 2:numtimesteps
  ...
  % New Operators
  Enew = interp2_matrix(x1d, y1d, cpx, cpy, p, bandnew);
  E = interp2_matrix(x1d, y1d, g(t - dt)/g(t).*cpx, g(t - dt)/g(t).*cpy, p, band);
  Eold = interp2_matrix(x1d, y1d, g(t - 2*dt)/g(t).*cpx, g(t - 2*dt)/g(t).*cpy, p, bandold);
  I = speye(size(Enew));
  L = laplacian_2d_matrix(x1d,y1d, order, bandnew);
  ...
  [unew, flagu] = gmres(Au,(2*uc - 1/2*uoldc) - 2*k*dt*(1 - g(t - dt)/xi).*uc + k*dt*(1 - g(t - 2*dt)/xi).*uoldc + 2*dt.*(a - uc + uc.*uc.*vc) - dt.*(a - uoldc + uoldc.*uoldc.*voldc), 10, tol, maxit);%, Lu, Uu, u);
  [vnew, flagv] = gmres(Av,(2*vc - 1/2*voldc) - 2*k*dt*(1 - g(t - dt)/xi).*vc + k*dt*(1 - g(t - 2*dt)/xi).*voldc + 2*dt.*(b - uc.*uc.*vc) - dt.*(b - uoldc.*uoldc.*voldc), 10, tol, maxit);
