...
K = log(2) / (10*numtimesteps); %scaled to kt
k = log(2)/(10*Tf); %scaled to t
...
% function cpArc for finding the closest points on a circle arc
cpf = @(x,y) cpArc(x, y, exp(k*dt)*R, cen, 0, pi);
paramf = @(N) paramArc(N, exp(k*dt)*R, cen, 0, pi);
cpf0 = @(x,y) cpArc(x, y, R, cen, 0, pi);
paramf0 = @(N) paramArc(N, R, cen, 0, pi);

[cpx, cpy, dist, bdy] = cpbar_2d(xx,yy,cpf);
[cpx0, cpy0, dist0, bdy0] = cpbar_2d(xx,yy,cpf0);
...
I = speye(size(Enew));
Ic = I;
Ic(logical(bdy),:) = -Ic(logical(bdy),:);
Enew(logical(bdy),:) = -Enew(logical(bdy),:);
...
[unew, flagu] = gmres(A,(Ic*uc - (k*dt.*uc)), 10, tol, maxit);%, Lu, Uu, u);
uold = u;
u = unew;
...
for kt = 2:numtimesteps
  ...
  % Band and boundary trickle down
  bandold = band;
  band = bandnew;
  ...
  % New Surface
  cpf = @(x,y) cpArc(x, y, R*exp(k*t), cen, 0, pi);
  paramf = @(N) paramArc(N, R*exp(k*t), cen, 0, pi);
  
  [cpx, cpy, dist, bdynew] = cpbar_2d(xx,yy,cpf);
  cpx = cpx(:); cpy = cpy(:);
  
  bandnew = find(abs(dist) <= bw*dx);
  cpx = cpx(bandnew); cpy = cpy(bandnew);
  x = xx(bandnew); y = yy(bandnew);
  bdynew = bdynew(bandnew);
  ...
  Eold2 = Eold;
  Enew(logical(bdynew),:) = -Enew(logical(bdynew),:);
  %E(logical(bdynew),:) = -E(logical(bdynew),:);
  %Eold(logical(bdynew),:) = -Eold(logical(bdynew),:);
  I = speye(size(Enew));
  Ic = I;
  Ic(logical(bdynew),:) = -Ic(logical(bdynew),:);
  L = laplacian_2d_matrix(x1d,y1d, order, bandnew);
  ...
  [unew, flagu] = gmres(A,(Ic*(2*uc - 1/2*uoldc) + (- 2*k*dt.*(uc) + k*dt.*(uoldc))), 10, tol, maxit);%, Lu, Uu, u);
  ...
  if ( (kt < 10) || (mod(kt,1/10/dt) == 0) || (kt == numtimesteps) )
    ...
    clf;
    circplot = Eplot*unew;
    exactplot = exp(-k*t)*(exp((exp(-2*k*t) - 1)/(2*R^2*k)))*sin(thetas); %Exact solution for fixed domain
